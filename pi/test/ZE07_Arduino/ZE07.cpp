...
ZE07CO_Sensor::ZE07CO_Sensor(HardwareSerial* Serial)
    : mySerial(Serial),	receivedFlag(0)
//read the uart signal by hardware uart,such as D0
{}

ZE07CO_Sensor::ZE07CO_Sensor(SoftwareSerial* Serial)
    : mySerial(Serial), receivedFlag(0)	
//read the uart signal by software uart,such as D10
{}
...
uint8_t ZE07CO_Sensor::checkSum()	
{
    byte sum = 0;
    for(int i = 1; i < length-1; i ++)
		sum += receivedCommandStack[i];
...
bool ZE07CO_Sensor::available(uint16_t timeout)		//new data was recevied
{
    if (timeout > 0)
    {
	...
	if ((receivedCommandStack[MAXLENGTH-1]==checkSum()) && (_status = STATUS_OK))
    {
